...
import { regexGroupPathReader, regexGroupPathsReader } from '../src/index';
  ...
  describe('group matches w/1 file', () => {
    ...
    testArgs.forEach((testArg) => {
      it(`read stream from ${testArg.filePath}, args: ${JSON.stringify(testArg)}`, async () => {
        const result = await regexGroupPathReader({ path: testArg.filePath }, testArg.regex, testArg.streamOptions);
  ...
  describe('group matches w/many files', () => {
    ...
    testArgs.forEach((testArg) => {
      it(`read stream from ${testArg.glob}, args: ${JSON.stringify(testArg)}`, async () => {
        ...
        const results = await regexGroupPathsReader(objs, testArg.regex, testArg.streamOptions);
