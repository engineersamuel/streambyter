...
import { regexStreamReader, regexStreamsReader } from '../src/index';
  ...
  describe('group matches w/1 file', () => {
    ...
    testArgs.forEach((testArg) => {
      it(`read stream from ${testArg.filePath}, args: ${JSON.stringify(testArg)}`, async () => {
        ...
        const result = await regexStreamReader({ filePath: testArg.filePath, stream }, testArg.regex);
        console.log({ filePath: result.filePath, result: result.result });
  ...
  describe('group matches w/many files', () => {
    ...
    testArgs.forEach((testArg) => {
      it(`read stream from ${testArg.glob}, args: ${JSON.stringify(testArg)}`, async () => {
        ...
        const results = await regexStreamsReader(objs, testArg.regex);
